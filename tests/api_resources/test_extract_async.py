...
class TestExtractAsync:
    ...
    def test_method_create(self, client: Reducto) -> None:
        extract_async = client.extract_async.create(
            input="string",
        )
        assert_matches_type(AsyncExtractResponse, extract_async, path=["response"])
    ...
    def test_raw_response_create(self, client: Reducto) -> None:
        response = client.extract_async.with_raw_response.create(
            input="string",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        extract_async = response.parse()
        assert_matches_type(AsyncExtractResponse, extract_async, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    def test_streaming_response_create(self, client: Reducto) -> None:
        with client.extract_async.with_streaming_response.create(
            input="string",
        ) as response:
...
class TestAsyncExtractAsync:
    ...
    async def test_method_create(self, async_client: AsyncReducto) -> None:
        extract_async = await async_client.extract_async.create(
            input="string",
        )
        assert_matches_type(AsyncExtractResponse, extract_async, path=["response"])
    ...
    async def test_raw_response_create(self, async_client: AsyncReducto) -> None:
        response = await async_client.extract_async.with_raw_response.create(
            input="string",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        extract_async = await response.parse()
        assert_matches_type(AsyncExtractResponse, extract_async, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncReducto) -> None:
        async with async_client.extract_async.with_streaming_response.create(
            input="string",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
