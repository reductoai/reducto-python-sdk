...
from reducto._types import Base64FileInput, omit, not_given
...
from reducto._compat import PYDANTIC_V1
...
async def test_pydantic_nested_objects(use_async: bool) -> None:
    model = ModelNestedObjects.construct(nested={"foo": "reducto"})
    assert isinstance(model.nested, MyModel)
    assert cast(Any, await transform(model, Any, use_async)) == {"nested": {"foo": "reducto"}}
